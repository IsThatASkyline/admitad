...
from .services import process_html, process_url
...
class HtmlToPdfApi(APIView):
    def post(self, request):
        try:
            ...
            return Response(
                {
                    "detail": "В теле запроса необходимо указать ключ 'file' и с ним передать html-файл"
                }
            )
...
class UrlToPdfApi(APIView):
    def post(self, request):
        try:
            ...
            return Response(
                {
                    "detail": "В теле запроса необходимо указать ключ 'url' и с ним передать ссылку на страницу"
                }
            )
